...
global a1;global a2;global a4;global a5;global a6;global a7; global f;global fv;global h_upk;global cd1;
 global aa1;global b1;global cc1;global d1;global e1;global f1;global g1;global h1;global i1;global j1;global k1;global l1;global m1;global n1;global o1;global p1;global q1;global rr1;global s1;global t1;global u1;global v1;%声明公共变量%
...
a1(1,1)=c1{1,10}/(c1{1,10}+1);a1(1,2)=c2{1,10}/(c2{1,10}+1); %提取k因子，生成复合K’因子%
a2(1,1)=a1(1,1)*c1{1,11}+2;a2(2,1)=(1-a1(1,1))*c1{1,11}+2;
a2(1,2)=a1(1,2)*c2{1,11}+2;a2(2,2)=(1-a1(1,2))*c2{1,11}+2;%存储各外骨骼上下层的厚度%
  ...
  if (0<=h_up)&&(h_up<=min(a4(2,1),a4(1,2)))&&(0<=h_down)&&(h_down<=min(a4(2,2),a4(1,3))) %判断是否超界%
    ...
    x=fsolve(@(x)myfunk(x),14,optimset('Display','off'));
     ...
     x=fsolve(@(x)myfun3(x),3,optimset('Display','off'));
      ...
      plot(x,y1);
      xlabel('MCP的运动角度/°');ylabel('PIP的运动角度/°');legend('MCP-PIP弯曲耦合仿真实验曲线');
      subplot(1,2,2);
      y2=1.25*x;
      plot(x,y2);
       xlabel('MCP的运动角度/°');ylabel('PIP的运动角度/°');legend('MCP-PIP弯曲采样逼近曲线');
      hold on;
       plot(x,y2);
     k=ckk;
     dv=y1-y2;
